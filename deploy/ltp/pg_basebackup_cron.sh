...
#                                  Postgres TCP connection. PGUSER must
#                                  have REPLICATION (granted by bootstrap).
#                                  Cron runs this as root locally so the
#                                  CephFS write succeeds.
...
# Run as root locally (CephFS only honors uid=0); authenticate as ma3user
# over TCP using its REPLICATION grant + password.
...
cat > "$dest/manifest.json.tmp" <<JSON
{
  ...
  "start_lsn": "$start_lsn",
  "pg_version": $(psql -h "$PGHOST" -p "$PGPORT" -U "$PGUSER" -d "$PGDATABASE" -tAc 'SHOW server_version_num;' 2>/dev/null || echo 0)
}
JSON
