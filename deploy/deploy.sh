...
if [[ "${DEPLOY_SOURCE}" == "git" ]]; then
  ...
  REMOTE_PREPARE="${REMOTE_DIR}/bin/prepare_git_release.sh"
...
# ---------------------------------------------------------------------------
  ...
  CADDY_RELOAD_CMD_B64="$(printf '%s' "${CADDY_RELOAD_CMD}" | base64 | tr -d '\n')"
  PUBLIC_SMOKE_ARG="${BLUE_GREEN_PUBLIC_SMOKE_URL:-__MA3_EMPTY__}"
  REMOTE_DEPLOY="${APP_DIR}/deploy/common/deploy_release_remote.sh"
  $SSH "${REMOTE_USER}@${REMOTE_HOST}" bash "${REMOTE_DEPLOY}" \
    "${REMOTE_DIR}" "${APP_DIR}" "${DEPLOY_SOURCE}" \
    "${DEPLOY_GIT_COMMIT}" "${DEPLOY_GIT_COMMIT_FULL}" \
    "${MA3_PORT}" "${UVICORN_HOST}" "${REQUIRE_VECTOR}" "${HEALTHZ_TIMEOUT}" \
    "${EXPECT_INSTANCE_ID}" "${EXPECT_PUBLIC_BASE_URL}" \
    "${ASSERT_DEV_AUTH_OFF}" "${ASSERT_NO_LAN_PROXY}" \
    "${BLUE_GREEN}" "${BLUE_GREEN_PORT_A}" "${BLUE_GREEN_PORT_B}" \
    "${BLUE_GREEN_DRAIN_SEC}" "${CADDY_UPSTREAM_FILE}" \
    "${CADDY_RELOAD_CMD_B64}" "${PUBLIC_SMOKE_ARG}"
