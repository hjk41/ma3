...
> **已拍板**（[ADR 001–014](decisions/)，摘要见 [architecture-decisions.md](architecture-decisions.md)）：
> - SaaS 多租户为首要形态；LAN = dev profile（同一 binary）
> - Agent 面 = **MCP + policy**，删除 `install.sh` / CLI plugin
> - 全部 library 默认 active 写入（后由 [write-buffer.md](../03-backend/write-buffer.md) 增加 buffered 窗口）
> - 默认启用 vector，`MA3_DISABLE_EMBEDDINGS=1` 可关
> - Observatory（只读：cases、search、explain）；**仅产品管理员可访问**（见 [portal-permissions.md](../04-frontend/portal-permissions.md)）
> - 版本语义 = `protocol_version` + `skill_bundle_version`（Scheme B）
> - org 表进 v1，单租户可隐式 default org；monorepo；eval 在 monorepo、非发布物

## 1. 系统上下文

> 产品角色与价值主张见 [pitch.md](../01-product/pitch.md) §为谁而建、§维护分层。
...
```text
（人）浏览器 `/ui/keys/` 自助创建 API key（见 [getting-started.md](../05-agent/getting-started.md)）
...
| `ma3_case` | case 展开 |
| `ma3_report` | 写（`report_kind` + buffer 语义见 [writes-audit-and-deletion.md](../03-backend/writes-audit-and-deletion.md)、[write-buffer.md](../03-backend/write-buffer.md)） |
...
> `ma3_search_explain` 已**删除**、`ma3_context.include_explain` 已**移除**——排序分解只经内部 / Observatory 暴露（[search-and-ranking.md](../03-backend/search-and-ranking.md)，防刷榜）。
...
单租户部署使用 **`org_default`**；公共 library 与团队 library **同一套模型**（[pitch.md](../01-product/pitch.md)：Community, not silo）。完整 ACL 模型见 [authorization-and-libraries.md](../03-backend/authorization-and-libraries.md)。
...
- 搜索污染 → write buffer + ranking（[search-and-ranking.md](../03-backend/search-and-ranking.md)）+ **维护者 Agent** 日常扫描 + **人 — 维护者** mark invalid
- Agent 误写 → policy 强制写前校验 + buffer 窗口内作者可撤回（[write-buffer.md](../03-backend/write-buffer.md)）
- 隐私 / 价值观 → **人 — 维护者** 最终清除
...
| **Minimal** | `MA3_DISABLE_EMBEDDINGS=1` | 无 GPU、无 HF 网络（**仍走统一 GTN 排序管线**，见 [search-and-ranking.md](../03-backend/search-and-ranking.md)） |
...
- API keys：`ma3k_…` per principal + per-library grants；MCP 数据路径**只认 `X-API-Key`**（[authorization-and-libraries.md](../03-backend/authorization-and-libraries.md)）
- Admin：`MA3_AUTH_ADMIN_USERS`；**Authing 启用且 admin 白名单为空 → 拒绝启动**（[portal-permissions.md](../04-frontend/portal-permissions.md)）
...
1. **用户门户 `/ui/me/*` 等**（默认落地）— 以 principal 为中心：库权限、贡献、投票、API keys。规格见 [portal-permissions.md](../04-frontend/portal-permissions.md) + [information-architecture.md](../04-frontend/information-architecture.md)。
...
见 [deployment.md](../06-operations/deployment.md)（profile 总览）与 [deployment-authing.md](../06-operations/deployment-authing.md)（Authing 配置）。

| Profile | 优先级 |
|---------|--------|
| saas | **首要** |
| lan | dev / 自建 |
| ltp | legacy，非 v1 core |

## 12. ADR 索引

| ADR | 主题 | 状态 |
|-----|------|------|
| [001](decisions/001-saas-primary-deploy.md) | SaaS 首要 | Accepted |
| [002](decisions/002-active-default-writes.md) | 写回默认 active | Accepted |
| [003](decisions/003-mcp-only-agent-surface.md) | MCP + policy，无 CLI | Accepted |
| [004](decisions/004-vector-default-optional-off.md) | Vector 默认开 | Accepted |
| [005](decisions/005-observatory-ui-scope.md) | Observatory 范围 | Accepted |
| [006](decisions/006-hook-candidates-local-first.md) | Hook 候选本地优先 | Accepted |
| [007](decisions/007-v1-milestones-and-observatory-governance.md) | 里程碑与 Observatory 治理 | Accepted |
| [008](decisions/008-maintainer-human-or-agent.md) | 维护者：人与 Agent | Accepted |
| [009](decisions/009-client-sync-scheme-b.md) | Client sync Scheme B | Accepted |
| [010](decisions/010-authing-social-login.md) | Authing 社交登录 | Accepted |
| [011](decisions/011-kb-access-and-org-isolation.md) | KB 访问与 org 隔离 | Accepted |
| [012](decisions/012-billing-and-quotas.md) | 付费套餐与配额 | Accepted |
| [013](decisions/013-write-confirmation-audit-delete.md) | 写入确认、审计、删除 | Accepted |
| [014](decisions/014-mcp-error-self-correction.md) | MCP 错误可自纠 | Accepted |
