...
> **状态**：实现完成（2026-07-03），已部署 192.168.31.202
...
MCP 宿主（Cursor / Claude Code / Codex 等）只把 JSON-RPC `error.message` 传给模型；
`error.data` 往往被丢弃。这与 `structuredContent` 不可见是**同一类**宿主行为
（对策相同：把关键信息放进宿主一定展示的字段）。

**实证**：评测中 Claude 把 `ma3_report` payload 误套进 `arguments`（照抄
`ma3_validate` 的 `{tool_name, arguments}` 信封），只读到 `-32602 "Invalid params"`，
连试 6 次相同错误后放弃、未写回。修复后同场景 Agent 一次写回成功。
...
| `-32601`（映射自 404） | 未知工具 / record / case 不存在 | `HTTPException.detail`（如 `unknown tool: X`、`record not found`） |
...
| `-32001`（映射自 403） | 权限不足 | `HTTPException.detail`（如 `writer access required`） |
| `-32029`（映射自 429） | 限流 | `HTTPException.detail` |
| `-32000`（其它 5xx/HTTPException） | 兜底 | `HTTPException.detail` |

**规则**：
- HTTPException 路径天然满足约束——`str(detail)` 即 message；因此 detail 文案必须本身可操作。
- 非 HTTPException 路径（parse / invalid-request / 三类 invalid-params / method-not-found /
  invalid-credentials）**必须**显式把 detail 折叠进 message。

---

## 3. 校验错误摘要器

`routes_mcp.py::_summarize_validation_errors(tool_name, errors)` 把 Pydantic
`errors()` 折叠成单条可操作 message。
...
分桶:
  ...
  other    ← 其余                                            → "loc: msg"
构造:
  prefix = "Invalid params for {tool_name}"（无 tool_name 时 "Invalid params"）
  段落 = [
...
| 字段 | 出现于 | 内容 |
|------|--------|------|
| `validation_errors` | 校验类 -32602 | Pydantic `errors()` 原始列表（`loc`/`type`/`msg`） |
| `schema_hint` | 校验类 -32602 | 指向 `tools/list` inputSchema + `ma3_validate` |
| `tool_name` | 校验类 -32602 | 目标工具 |
| `status_code` | 由 HTTP 映射 | 原 HTTP 状态码（401/403/404/429…） |
...
- 工具内异常一律转 `HTTPException(detail=<安全文案>)` 或 `ValueError(<安全文案>)`；**禁止**把裸异常字符串直接塞进 message。
- `unknown tool` / `not found` 类 message 只回显调用者已提供的标识符，不泄露其它库/记录的存在性（与 [10-write-audit-and-delete.md](10-write-audit-and-delete.md) 的 existence-oracle 约束一致）。
...
| 测试 | 断言 |
|------|------|
| `test_...report`（既有，强化） | -32602 且 `message` 含 `ma3_report` + `missing required field` + `outcome`/`result_summary` |
| `test_ma3_report_envelope_confusion_message` | message 含 `unexpected field` + `arguments` + `FLAT payload` + `ma3_validate` |
...
| 文件 | 职责 |
|------|------|
| `code/server/app/api/routes_mcp.py` | `_jsonrpc_error`、`_summarize_validation_errors`、所有错误路径构造 |
| `code/server/app/models/mcp.py` | `McpToolValidationError`（携带 `errors[]`） |
...
- [x] 五类非 HTTPException 错误 message 均可自纠（curl 实测 202）
- [x] 信封混淆 message 定向提示 FLAT payload
- [x] `data.validation_errors` 保留
- [x] 88 本地测试通过；202 部署验证 10/10
- [x] 清记忆 Claude 实测：`ma3_report` 一次写回成功（`vk_0dc976edbe9b`）
...
| 文档 | 关系 |
|------|------|
| [ADR-014](../adr/014-mcp-error-self-correction.md) | 决策 |
| [ADR-003](../adr/003-mcp-only-agent-surface.md) | MCP 是唯一 Agent 面 |
| [10-write-audit-and-delete.md](10-write-audit-and-delete.md) | existence-oracle / 不泄露存在性 |
| [08-kb-access-and-org-isolation.md](08-kb-access-and-org-isolation.md) | 401/403 语义来源 |
