...
> **测试**：`tests/integration/test_display_name_registration.py`
...
- **Principal ID**（`user:{sso_sub}`）永久不变，在 **设置页** 只读展示
...
| `/ui/me/settings/` | 只读显示名 + Principal ID |
| `/ui/me/` | 概览（仅 avatar + 显示名，无编辑入口） |

---

## 5. 集成测试映射

见 `docs/acceptance/v1-display-name-registration.md`。
