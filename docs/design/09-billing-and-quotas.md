# 09 — 付费套餐与配额（Billing & Quotas）

> **ADR**：[ADR-012](../adr/012-billing-and-quotas.md)  
> **前置**：[08-kb-access-and-org-isolation.md](08-kb-access-and-org-isolation.md) Phase 1–3  
> **状态**：设计定稿（2026-07-02）  
> **Pitch**：[kb-access-and-team-pitch.md](../pitch/kb-access-and-team-pitch.md)

---

## 1. 目标

| 目标 | 说明 |
|------|------|
| 三档套餐 | Free / Pro / Team，五维 quota 映射到 `plans` 表 |
| billing_account | 用量与资源的计费归属一等实体 |
| implicit personal org | 每用户自动 personal org + free billing account |
| 贡献安全 | 公共库写回、ma3_report 不计费惩罚 |
| v1 无 Stripe | `plan_code` 手工设置；schema 预留 provider 字段 |

---

## 2. 概念模型

```text
Principal (user)
    │
    ├── org_personal_{sub}  ── billing_account(plan=free|pro)
    │         └── libraries (personal, max N)
    │
    └── org_members ── org_company_x ── billing_account(plan=team)
              └── libraries (org visibility)

api_key.billing_account_id  →  read usage 计入此处
library.org_id              →  storage / library count 计入 owner org 的 billing_account
```

### 2.1 Read unit 计量

| 工具 | Billable | 规则 |
|------|----------|------|
| `ma3_context` | 是 | 1 + max(0, ceil(records_returned/10) - 1) |
| `ma3_case` | 是 | 同上 |
| `ma3_report` | **否** | 贡献路径 |
| `ma3_feedback` | **否** | |
| `ma3_validate` | **否** | |
| `ma3_list_drafts` / `ma3_review_record` | **否** | maintainer |
| `ma3_create_key` 等 mgmt | **否** | |

仅 **2xx 成功**响应计 billable；4xx 记 observability 不进 quota。

### 2.2 Storage 计量

- 计数：**active + draft** records（`invalid` 不计或半计，v1 不计）
- 归属：library → owner org → org 的 `billing_account`
- **public library（`lib_default`）**：owner 视为 platform；写入 **不占** writer 任何 quota
- 字段：`libraries.active_record_count`（维护）；可选 `storage_bytes` 汇总

---

## 3. 数据模型

### 3.1 `plans`

```sql
CREATE TABLE plans (
  code TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  scope TEXT NOT NULL CHECK (scope IN ('personal', 'org')),
  allow_readonly_grants INTEGER NOT NULL DEFAULT 0,
  max_libraries INTEGER NOT NULL,
  max_records_per_library INTEGER NOT NULL,
  max_records_total INTEGER NOT NULL,
  monthly_read_units INTEGER NOT NULL,
  daily_read_units INTEGER,
  rate_limit_per_min INTEGER NOT NULL DEFAULT 60,
  included_seats INTEGER NOT NULL DEFAULT 1,
  max_external_grants_per_library INTEGER NOT NULL DEFAULT 2,
  max_keys INTEGER NOT NULL DEFAULT 10,
  overage_mode TEXT NOT NULL DEFAULT 'block' CHECK (overage_mode IN ('block', 'allow')),
  created_at TEXT NOT NULL
);
```

**Seed（v1 起点）**：

| code | scope | RO | lib | rec/lib | rec total | read/mo | seats | keys |
|------|-------|----|-----|---------|-----------|---------|-------|------|
| free | personal | 0 | 1 | 1000 | 3000 | 10000 | 1 | 10 |
| pro | personal | 1 | 5 | 10000 | 50000 | 100000 | 1 | 50 |
| team | org | 1 | 10 | 100000 | 100000 | 500000 | 5 | 200 |

Team 的 `max_records_per_library` / `max_records_total` 均为 org **pooled**。

### 3.2 `billing_accounts`

```sql
CREATE TABLE billing_accounts (
  id TEXT PRIMARY KEY,
  owner_type TEXT NOT NULL CHECK (owner_type IN ('principal', 'org')),
  owner_id TEXT NOT NULL,
  plan_code TEXT NOT NULL REFERENCES plans(code),
  status TEXT NOT NULL DEFAULT 'active'
    CHECK (status IN ('active', 'past_due', 'cancelled')),
  current_period_start TEXT NOT NULL,
  current_period_end TEXT NOT NULL,
  provider TEXT,
  provider_customer_id TEXT,
  created_at TEXT NOT NULL,
  UNIQUE (owner_type, owner_id)
);
CREATE INDEX idx_billing_accounts_plan ON billing_accounts (plan_code);
```

### 3.3 `quota_overrides`

```sql
CREATE TABLE quota_overrides (
  billing_account_id TEXT NOT NULL,
  quota_key TEXT NOT NULL,
  value INTEGER NOT NULL,
  reason TEXT,
  expires_at TEXT,
  PRIMARY KEY (billing_account_id, quota_key)
);
```

`quota_key` 示例：`monthly_read_units`、`max_libraries`、`included_seats`。

### 3.4 `usage_events`（append-only）

```sql
CREATE TABLE usage_events (
  id TEXT PRIMARY KEY,
  occurred_at TEXT NOT NULL,
  billing_account_id TEXT NOT NULL,
  principal_id TEXT,
  api_key_id TEXT,
  org_id TEXT,
  library_id TEXT,
  tool_name TEXT NOT NULL,
  operation TEXT NOT NULL CHECK (operation IN ('read', 'write', 'admin')),
  units INTEGER NOT NULL DEFAULT 0,
  records_returned INTEGER,
  status_code INTEGER,
  created_at TEXT NOT NULL
);
CREATE INDEX idx_usage_events_ba_time ON usage_events (billing_account_id, occurred_at);
```

### 3.5 `usage_daily` / `usage_monthly`

```sql
CREATE TABLE usage_daily (
  billing_account_id TEXT NOT NULL,
  date TEXT NOT NULL,
  metric TEXT NOT NULL,
  value INTEGER NOT NULL DEFAULT 0,
  PRIMARY KEY (billing_account_id, date, metric)
);

CREATE TABLE usage_monthly (
  billing_account_id TEXT NOT NULL,
  period_start TEXT NOT NULL,
  metric TEXT NOT NULL,
  value INTEGER NOT NULL DEFAULT 0,
  PRIMARY KEY (billing_account_id, period_start, metric)
);
```

`metric`：`read_units`、`reports`（观测用，非 billable）。

### 3.6 扩展 ADR-011 表

**`organizations`**：

| 列 | 说明 |
|----|------|
| `kind` | `personal` \| `team`（implicit personal org 标记） |
| `billing_account_id` | FK → billing_accounts |

**`api_keys`**（ADR-011 基础上）：

| 列 | 说明 |
|----|------|
| `billing_account_id` | NOT NULL；read usage 归属 |

**`libraries`**：

| 列 | 说明 |
|----|------|
| `active_record_count` | 维护计数 |
| `storage_bytes` | 可选 |

**`org_members`**：

| 列 | 说明 |
|----|------|
| `seat_status` | `active` \| `pending` \| `removed` |
| `joined_at` | ISO8601 |

---

## 4. Bootstrap 流程

用户首次 Authing 登录（`ensure_user_principal` 之后）：

```text
1. IF NOT EXISTS org WHERE kind='personal' AND owner=principal:
     CREATE org_personal_{sub}, kind=personal
     CREATE billing_account(owner=org, plan=free)
     INSERT org_members(org, principal, role=admin, seat_status=active)
2. 返回 principal + personal_org_id + billing_account_id
```

显式创建 Team org（Observatory / 未来 MCP）：

```text
1. CREATE org, kind=team
2. CREATE billing_account(owner=org, plan=team)  -- 或 free 待升级
3. creator → org_members admin
4. 邀请第 2 人时 check included_seats（free personal org 永远 1 seat → 403）
```

---

## 5. Enforcement 伪代码

### 5.1 Read-only grant（key 创建）

```python
def validate_key_grants(billing_account_id: str, grants: list[KeyGrant]) -> None:
    ba = load_billing_account(billing_account_id)
    plan = load_plan(ba.plan_code)
    for g in grants:
        if not g.can_write:
            if not plan.allow_readonly_grants:
                g.can_write = True  # FORCE + audit log
        assert_subset_of_entitlement(ba.owner, g.library_id)
```

### 5.2 Storage（ma3_report）

```python
def check_storage_quota(library_id: str) -> QuotaCheck:
    if library_id == settings.public_library_id:
        return QuotaCheck(ok=True)  # platform bears cost
    ba = billing_account_for_library(library_id)
    usage = count_active_records(ba)
    limit = effective_quota(ba, "max_records_total")
    if usage >= limit:
        # 超 cap：该库转只读——禁写，读不受影响
        raise HTTPException(403, "library is read-only: storage quota exceeded")
    if usage >= limit * 0.8:
        return QuotaCheck(ok=True, warn=True, pct=usage/limit)
    return QuotaCheck(ok=True)
```

### 5.3 Read usage（MCP 读工具）

```python
def record_read_usage(auth: McpAuthContext, tool: str, units: int) -> None:
    ba_id = auth.key_billing_account_id
    append_usage_event(...)
    increment_usage_monthly(ba_id, "read_units", units)
    if over_monthly_limit(ba_id):
        raise HTTPException(429, headers={"Retry-After": "3600"})
```

### 5.4 Org seats

```python
def accept_invite(org_id: str, principal_id: str) -> None:
    org = load_org(org_id)
    if org.kind == "personal":
        raise HTTPException(403, "personal org cannot add members")
    ba = billing_account_for_org(org_id)
    seats = count_active_seats(org_id)
    limit = effective_quota(ba, "included_seats")
    if seats >= limit:
        raise HTTPException(403, "seat quota exceeded; upgrade to Team")
```

---

## 6. MCP `structuredContent.quota`

每个 MCP 响应可选携带（与 `server` 块并列）：

```json
{
  "quota": {
    "billing_account_id": "ba_xxx",
    "plan": "free",
    "read_units": {"used": 8200, "limit": 10000, "period": "2026-07"},
    "storage": {"used": 2400, "limit": 3000, "scope": "personal"},
    "warnings": ["read_units at 82%"],
    "upgrade_url": "/ui/billing/"
  }
}
```

Agent policy 可要求：有 `warnings` 时转告用户。

---

## 7. 与 ADR-011 集成点

| ADR-011 位置 | ADR-012 钩子 |
|--------------|--------------|
| `entitlement_service.validate_key_grants` | 调用 `billing_service.allow_readonly()` |
| `org_service.create_library` | `billing_service.check_library_count()` |
| `org_service.invite_member` | `billing_service.check_seats()` |
| `mcp_tool_service` 读工具 | `billing_service.record_read()` + quota warn |
| `mcp_tool_service` ma3_report | `billing_service.check_storage()` |
| `ma3_whoami` | 返回 plan + quota 摘要 |
| `ma3_doctor` | `billing_schema_ok`, `usage_rollup_lag` |

**`owner_has_readonly_privilege`**（ADR-011）替换为：

```python
def allow_readonly_grants(billing_account_id: str) -> bool:
    plan = plan_for_billing_account(billing_account_id)
    return bool(plan.allow_readonly_grants)
```

---

## 8. Observatory / Admin（Phase B4）

| 页面 | 功能 |
|------|------|
| `/ui/billing/` | 当前 plan、quota 使用率、升级 CTA |
| `/ui/billing/upgrade` | 展示 Pro / Team；v1 手工联系或 admin API |
| Admin API | `PATCH /admin/billing_accounts/{id}` 设 `plan_code`（platform maintainer） |

Stripe webhook 占位：`POST /webhooks/stripe` → v1.1。

---

## 9. 分阶段实现

### Phase B1 — Schema & bootstrap

- 创建 §3 表；seed `plans`
- `organizations.kind`、`billing_account_id`
- 登录 bootstrap personal org + billing account
- **无 enforcement**

**验收**：新用户登录后有 personal org + ba；migration 可重复。

### Phase B2 — Metering

- `billing_service.record_read_usage()` 挂读工具
- rollup job：`usage_daily` / `usage_monthly`
- 维护 `libraries.active_record_count`
- Observatory / `ma3_doctor` 展示使用率

**验收**：integration test 读 10 次后 `usage_monthly.read_units == 10`。

### Phase B3 — Enforcement

- RO grant、library count、storage、seats、read 429
- `structuredContent.quota` warnings
- 降级：past_due 限写不限读

**验收**：`test_billing_enforcement.py` 全绿。

### Phase B4 — UI & Stripe stub

- Observatory billing 页
- Admin plan 变更 API
- Stripe webhook 路由空实现 + 文档

---

## 10. 测试策略

| 文件 | 覆盖 |
|------|------|
| `tests/unit/test_billing_quotas.py` | plan limits、RO gate、read unit 计算 |
| `tests/unit/test_billing_storage.py` | public 写不占 quota；org 写占 org quota |
| `tests/integration/test_billing_enforcement.py` | 超 read 429；personal org 第二 member 403 |

---

## 11. v1.1 开放问题

| 项 | 说明 |
|----|------|
| Team overage 默认开/关及单价 | 商业策略 |
| Pro 是否含「轻量 Team」 | 产品包装 |
| Contribution credit | 写回换 read quota |
| 向量检索档位 | Fable 建议独立 meter |
| Stripe Price ID 与 plan 映射 | 支付接入 |

---

## 12. 相关文档

| 文档 | 关系 |
|------|------|
| [ADR-012](../adr/012-billing-and-quotas.md) | 决策 |
| [08-kb-access-and-org-isolation.md](08-kb-access-and-org-isolation.md) | ACL 前置 |
| [kb-access-and-team-pitch.md](../pitch/kb-access-and-team-pitch.md) | 商业叙事 |
| [05-doc-code-mapping.md](05-doc-code-mapping.md) | 代码映射 |
