...
> **前置**：[08-kb-access](authorization-and-libraries.md)、[09-billing](billing-and-quotas.md)  
> **状态**：设计定稿；`confirmation` 语义按 [13](../05-agent/getting-started.md) §10 修订（缺省不再 400）；发布时序见 [16-library-write-buffer.md](write-buffer.md)
...
| 库 | 默认 name |
|----|-----------|
| personal | `{display_name} 的个人库`（`ensure_personal_library` 随显示名同步，见 [17](../04-frontend/display-name-registration.md)） |
...
- `api_key_id` 是历史字符串，**不设外键**：key 被硬删后审计行保留原 id（[14](../04-frontend/api-keys-ui-and-api.md) §3）。
...
门户对应页：`/ui/me/writes/`（[22-user-portal-ui-layout.md](../04-frontend/information-architecture.md) §3.4）。
...
| `ma3_publish_record` | buffered 提前发布（见 [16](write-buffer.md)） |
