...
> **状态**：设计定稿；key 签发/生命周期的 v1 实现以 [13-self-service-onboarding.md](13-self-service-onboarding.md)、[14-api-key-lifecycle.md](14-api-key-lifecycle.md) 为准
...
| **付费 read-only** | 仅付费 plan 可创建 **只读** grant（见 [ADR-012](../adr/012-billing-and-quotas.md)、[09](09-billing-and-quotas.md)） |
...
| **Personal library** | `kind=personal`，`owner_principal_id` 指向用户；名为 `{display_name} 的个人库`（[10](10-write-audit-and-delete.md) §3） |
...
`id, name, entitlement (free|paid), created_at`（billing 扩展见 [09](09-billing-and-quotas.md) §3.6）
...
`ma3_report`：可选 `library_id`；缺省 → **owner 的 personal library**（`default_owned_personal_library`）；响应回显 `library_selection_reason`。verify/refute 跟随 `target_record_id` 所在库。详见 [10](10-write-audit-and-delete.md) §2、[13](13-self-service-onboarding.md) §10。

---

## 5. Key 管理界面

**v1 实现真源**：[13-self-service-onboarding.md](13-self-service-onboarding.md)（签发）+ [14-api-key-lifecycle.md](14-api-key-lifecycle.md)（生命周期）。要点：
...
| 审计 | 创建/删除/grant 变更写 application log；写入审计见 [10](10-write-audit-and-delete.md) |
...
| `entitlement` 字段 → `billing_account.plan_code` | [09](09-billing-and-quotas.md) |
...
| [09-billing-and-quotas.md](09-billing-and-quotas.md) | 付费套餐与配额 |
| [10-write-audit-and-delete.md](10-write-audit-and-delete.md) | 写入审计与删除 |
| [13-self-service-onboarding.md](13-self-service-onboarding.md) | 自助注册与 key 签发（v1 实现） |
| [14-api-key-lifecycle.md](14-api-key-lifecycle.md) | key 生命周期（v1 实现） |
| [15-user-portal.md](15-user-portal.md) | UI 侧 Stats/Enumerate/Mutate 三级能力 |
