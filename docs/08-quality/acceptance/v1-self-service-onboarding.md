...
- **Deploy tree note**: the running deployment root is `<DEPLOY_DIR>/` (code at `<DEPLOY_DIR>/code/server/`), not `~/ma3/` as literally written in §14 A10.
...
| A10 | Seed script in deploy tree | **PASS** | `ls <DEPLOY_DIR>/code/server/scripts/` → `seed_personal_library_key.py` (plus `e2e_authing_ui.py`, `migrate_legacy_pg.py`). Admin fallback available on the deployed host without a source checkout. |
...
4. **Doc path drift in §14 A10**: says `~/ma3/server/scripts/` but the deployment root on the LAN host is `<DEPLOY_DIR>/code/server/scripts/`. Worth fixing the design doc line.
