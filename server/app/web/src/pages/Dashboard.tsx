...
export function Dashboard() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-8 py-8">
      ...
      {anon ? (
        <Card title="Welcome">
          <p className="text-slate-600 mb-4">Sign in to manage your libraries and keys.</p>
        ...
        <>
          <div className="grid grid-cols-3 gap-3 mb-8">
            <Link
              to="/libs"
              className="p-4 border border-slate-200 rounded-xl hover:border-blue-300 bg-white"
            >
              <span className="text-2xl">📚</span>
              <span className="block text-sm font-medium text-slate-700">Libraries</span>
            </Link>
            <Link
              to="/keys"
              className="p-4 border border-slate-200 rounded-xl hover:border-blue-300 bg-white"
            >
              <span className="text-2xl">🔑</span>
              <span className="block text-sm font-medium text-slate-700">Keys</span>
            </Link>
            {whoami?.admin_bypass && (
              <Link
                to="/admin"
          ...
          <Card title="Effective libraries">
            <div className="space-y-2">
              {whoami?.libraries.map((library: any) => (
                ...
                >
                  <span>{library.name || library.library_id}</span>
                  <Badge
                    label={library.role}
                    color={
                      library.role === 'admin'
