...
def test_agent_ingest_default_redacts_detected_values(authed_client):
...
def test_agent_ingest_redaction_none_preserves_detected_values(authed_client):
    resp = authed_client.post("/agent/ingest", json=make_ingest_payload(
        problem=f"Use {HPCX_PATH} on {HOST_IP}; contact {EMAIL}; {TOKEN}",
        result_summary="Documented launch path",
        redaction_mode="none",
...
def test_knowledge_redaction_none_preserves_detected_values(authed_client):
...
def test_v2_agent_report_redaction_none_preserves_detected_values(authed_client):
