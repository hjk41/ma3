...
def test_login_flow_routes_and_principal_materialization(monkeypatch):
    ...
    login = asyncio.run(auth_login(_req("https://ma3.zhilicon.com/auth/login"), next="/me"))
    assert login.status_code == 302
    assert "auth.zhilicon.com/login" in login.headers["location"]
    callback = asyncio.run(auth_callback(_req("https://ma3.zhilicon.com/auth/callback"), gateway_token="fake", return_to="/me"))
